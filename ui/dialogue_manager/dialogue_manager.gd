...
@onready var mumble_audio: AudioStreamPlayer2D = $MumbleAudio
var current_idx = 0
...
func _process(delta: float) -> void:
	...
	if mumble_audio.stream and text_tween.is_running() and not mumble_audio.playing:
		mumble_audio.pitch_scale = randf_range(0.8, 1.2)
		mumble_audio.play()
...
func next_dialogue(): 
	...
	if text_tween: 
		text_tween.kill()
	mumble_audio.stream = dialogue.character.mumble
